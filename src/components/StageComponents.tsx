...
export const StageButtons: React.FC<IStageButtonsOwnProps> = ({ stageText, stages, handleStageClick }): JSX.Element => {
        ...
        <ButtonGroup color="primary" aria-label="outlined primary button group">
          {
            stages.map((stage, index) => {
              return <Button key={parseInt(stage)} onClick={() => handleStageClick(parseInt(stage))}>{parseInt(stage) + 1}</Button>
            })
          }
